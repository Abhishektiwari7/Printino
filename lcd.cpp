...
void Nokia105:: initDisplay(void) {
  ...
  #warning "I am using LINUX PLATFORM"
  DEBUG_INFO("...........Init Display.........%d",hwSPI);
  char SPIDEVICE_RES_lx[4];
  snprintf(SPIDEVICE_RES_lx, sizeof(SPIDEVICE_RES_lx), "%d", SPIDEVICE_RES);
  DEBUG_INFO("RES: %s", SPIDEVICE_RES_lx);
  unexport_gpio(SPIDEVICE_RES_lx);
  usleep(250000); //wait for sysfs to create gpio folders
  export_gpio(SPIDEVICE_RES_lx);
  usleep(250000); //wait for sysfs to create gpio folders
  set_gpio_direction("out", SPIDEVICE_RES_lx);
  
  hardwareSpiInit();//Spi Setup
          
  set_gpio_value("0", SPIDEVICE_RES_lx); 
  usleep(10000);  
  set_gpio_value("1", SPIDEVICE_RES_lx); 
  usleep(10000);      
  writeNokiaSPI(NOKIA105_SPLOUT,c);    // vTaskDelay,sleep out//0x11    
  usleep(10000);
...
//gpio_set_level((gpio_num_t)BACK_LIGHT, 1); //backlight
  ...
  #warning "I am using LINUX PLATFORM"
  char SPIDEVICE_BACKLIGHT_lx[4];
  snprintf(SPIDEVICE_BACKLIGHT_lx, sizeof(SPIDEVICE_BACKLIGHT_lx), "%d", BACK_LIGHT);
  DEBUG_INFO("BACKLIGHT: %s", SPIDEVICE_BACKLIGHT_lx);
  unexport_gpio(SPIDEVICE_BACKLIGHT_lx);
  usleep(250000); //wait for sysfs to create gpio folders
  export_gpio(SPIDEVICE_BACKLIGHT_lx);
  usleep(250000); //wait for sysfs to create gpio folders
  set_gpio_direction("out", SPIDEVICE_BACKLIGHT_lx);        
  set_gpio_value("1", SPIDEVICE_BACKLIGHT_lx); 
