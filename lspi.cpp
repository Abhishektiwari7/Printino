...
#include "lcd.h" //totalpixels
  ...
  #define GPIO_NUMBER "60" // Example GPIO pin number (P9_12)
  #define SYSFS_GPIO_DIR "/sys/class/gpio/"
  #define MAX_BUF 64

  char	_SPIDEVICE_CS[4],     //chip select activelow - string representation
        _SPIDEVICE_SDA[4],	   //Mosi /miso - string representation
        _SPIDEVICE_SCK[4],    //clock - string representation
        _hwSPI;            // 0:bit bang spi,1: hardware spi,2: none

  void set_gpio_direction(const char *direction, const char *gpionum) {
      int fd;
      char buf[MAX_BUF];
      snprintf(buf, sizeof(buf), SYSFS_GPIO_DIR "gpio%s/direction", gpionum);
      fd = open(buf, O_WRONLY);
      if (fd < 0) {
          DEBUG_ERROR("Error opening direction file");
          exit(1);
      }
  ...
  void set_gpio_value(const char *value,const char *gpionum) {
      int fd;
      char buf[MAX_BUF];
      snprintf(buf, sizeof(buf), SYSFS_GPIO_DIR "gpio%s/value", gpionum);
      fd = open(buf, O_WRONLY);
      if (fd < 0) {
          DEBUG_ERROR("Error opening value file");
          exit(1);
      }
  ...
  void export_gpio(const char *gpionum) {
      int fd = open(SYSFS_GPIO_DIR "export", O_WRONLY);
      if (fd < 0) {
          // Pin might already be exported, which is fine
          // DEBUG_ERROR("Error opening export file, pin might already be exported");
      } else {
          write(fd, gpionum, strlen(gpionum));
          close(fd);
      }
  }

  void unexport_gpio(const char *gpionum) {
      int fd = open(SYSFS_GPIO_DIR "unexport", O_WRONLY);
      if (fd < 0) {
          DEBUG_ERROR("Error opening unexport file");
      } else {
          write(fd, gpionum, strlen(gpionum));
          close(fd);
      }
  ...
  #define LCD_CS_High()    { set_gpio_value("1", _SPIDEVICE_CS);   }
  #define LCD_CS_Low()     { set_gpio_value("0", _SPIDEVICE_CS);   } 
  #define LCD_SDA_High()   { set_gpio_value("1", _SPIDEVICE_SDA);  } 
  #define LCD_SDA_Low()    { set_gpio_value("0", _SPIDEVICE_SDA);  } 
  #define LCD_SCK_High()   { set_gpio_value("1", _SPIDEVICE_SCK);  } 
  #define LCD_SCK_Low()    { set_gpio_value("0", _SPIDEVICE_SCK);  }

  void spiInit(uint8_t  _SID, uint8_t _SCLK, uint8_t _CS,uint8_t _hwSPI1) {
    DEBUG_INFO("Init BitBang SPI");
    // Convert GPIO numbers to string format
    snprintf(_SPIDEVICE_SDA, sizeof(_SPIDEVICE_SDA), "%d", _SID);
    DEBUG_INFO("SDA: %s", _SPIDEVICE_SDA);
    
    snprintf(_SPIDEVICE_SCK, sizeof(_SPIDEVICE_SCK), "%d", _SCLK);
    DEBUG_INFO("SCK: %s", _SPIDEVICE_SCK);
    
    snprintf(_SPIDEVICE_CS, sizeof(_SPIDEVICE_CS), "%d", _CS);
    DEBUG_INFO("CS: %s", _SPIDEVICE_CS);

    _hwSPI = _hwSPI1; //local to global
    unexport_gpio(_SPIDEVICE_CS);
    unexport_gpio(_SPIDEVICE_SDA);
    unexport_gpio(_SPIDEVICE_SCK);
    usleep(250000); //wait for sysfs to create gpio folders
    DEBUG_INFO("Exported GPIOs");
    export_gpio(_SPIDEVICE_CS);
    export_gpio(_SPIDEVICE_SDA);
    export_gpio(_SPIDEVICE_SCK);
    usleep(250000); //wait for sysfs to create gpio folders
    DEBUG_INFO("Set GPIO Directions");
    set_gpio_direction("out", _SPIDEVICE_CS);
    set_gpio_direction("out", _SPIDEVICE_SDA);
    set_gpio_direction("out", _SPIDEVICE_SCK);
    DEBUG_INFO("GPIO Directions Set to OUTPUT");
  }
