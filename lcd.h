...
//---------------------------PWM----------------------------------------------------------------------
const int BACK_LIGHT = 38; //GPIO for backlight
...
#define PINK              0xF81F
  ...
  #warning "I am using LINUX PLATFORM"
    ...
    static inline void getTimestamp(char* buffer, size_t len) {
    ...
    do { \
        if(level) { \
            ...
            const char* levelStr = (level == SHOW_ERROR) ? "ERROR" : \
                                   (level == SHOW_WARNING) ? "WARNING" : "INFO"; \
    ...
    #define DEBUG_INFO(fmt, ...) DEBUG_PRINT(SHOW_INFO, fmt, ##__VA_ARGS__)
    #define DEBUG_ERROR(fmt, ...) DEBUG_PRINT(SHOW_ERROR, fmt, ##__VA_ARGS__)
    #define DEBUG_WARNING(fmt, ...) DEBUG_PRINT(SHOW_WARNING, fmt, ##__VA_ARGS__)
    #define DEBUG(fmt, ...) DEBUG_ERROR(fmt, ##__VA_ARGS__)
